...
\begin{table}[ht]
	...
	\begin{tabular}{rrrr}
		\hline
		& Intercept ($\beta_0$) & EADS1TRUE ($\beta_1$) & EADS2TRUE ($\beta_2$) \\ 
		\hline
...
\lambda_i &= \mathrm{Rep.events}_i \cdot \exp(\beta_1 \cdot \mathrm{EADS1}_i + \beta_2 \cdot \mathrm{EADS2}_i + \theta),\\
\log(\lambda_i) &= \beta_1 \cdot \mathrm{EADS1}_i + \beta_2 \cdot \mathrm{EADS2}_i + \ln(\mathrm{Rep.events}_i) + \theta,\\
\mathrm{Deaths}_i &\sim \mathrm{Poisson}(\lambda_i)
\end{align*} 
...
\begin{table}[ht]
	...
	\begin{tabular}{rrrr}
		\hline
		& EADS1TRUE ($\beta_1$) & EADS2TRUE ($\beta_2$) & theta ($\theta$) \\ 
		\hline
